 ...
 * least one of the interface's methods is implemented.
 * <p>
 * 
 ...
 * place this into the application's classpath at startup. For example, if Burp
 * Suite is loaded from burp.jar, and BurpProxyExtender.jar contains the class
 * burp.BurpExtender, use the following command to launch Burp Suite and load
 * the IBurpExtender implementation:
 * <p>
 * 
 * <PRE>
 ...
 * </PRE>
 * 
 * (On Linux-based platforms, use a colon character instead of the semi-colon as
...
public interface IBurpExtender {
	 ...
	 * 
	 * @param args
	 *            The command-line arguments passed to Burp Suite on startup.
	 */
	 ...
	 *            An identifier which is unique to a single request/response
	 *            pair. This can be used to correlate details of requests and
	 *            responses and perform processing on the response message
	 *            accordingly.
	 * @param messageIsRequest
	 *            Flags whether the message is a client request or a server
	 *            response.
	 * @param remoteHost
	 *            The hostname of the remote HTTP server.
	 * @param remotePort
	 *            The port of the remote HTTP server.
	 * @param serviceIsHttps
	 ...
	 *            The requested URL.
	 * @param resourceType
	 *            The filetype of the requested resource, or a zero-length
	 *            string if the resource has no filetype.
	 * @param statusCode
	 *            The HTTP status code returned by the server. This value is
	 *            <code>null</code> for request messages.
	 * @param responseContentType
	 *            The content-type string returned by the server. This value is
	 *            <code>null</code> for request messages.
	 * @param message
	 ...
	 *            <code>ACTION_FOLLOW_RULES</code>. Set <code>action[0]</code>
	 *            to one of the other possible values to perform a different
	 *            action.
	...
	/**
	 * Causes Burp Proxy to present the message to the user for manual review or
	 * modification.
	 */
	...
	/**
	 * Causes Burp Proxy to present the message to the user for manual review or
	 * modification, and then make a second call to processProxyMessage.
	 */
	 ...
	 * be invoked by the implementation to perform various actions.
	 * 
	 ...
	 * 
	 * @param callbacks
	 *            An implementation of the <code>IBurpExtenderCallbacks</code>
	 *            interface.
	 */
	public void registerExtenderCallbacks(burp.IBurpExtenderCallbacks callbacks);

	/**
	 * This method is invoked immediately before Burp Suite exits. It allows
	 * implementations to carry out any clean-up actions necessary (e.g.
	 * flushing log files or closing database resources).
	 */
	 ...
	 * 
	 * @param toolName
	 *            The name of the Burp tool which is making the request.
	 * @param messageIsRequest
