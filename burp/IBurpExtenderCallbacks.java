 ...
 * <code>IBurpExtender</code> interface a set of callback methods which can be
 * used by implementations to perform various actions within Burp Suite.
 * 
 * If an implementation of <code>IBurpExtender</code> is loaded then on startup
 * Burp Suite will invoke the implementation's
 * <code>registerExtenderCallbacks</code> method (if present) and pass to the
 * implementation an instance of the <code>IBurpExtenderCallbacks</code>
 * interface. The implementation may then invoke the methods of this instance as
 * it sees fit in order to extend Burp Suite's functionality.
 * <p>
 */
...
public interface IBurpExtenderCallbacks {
	 ...
	 */
	public byte[] makeHttpRequest(String host, int port, boolean useHttps,
	 ...
	 *            The full HTTP request.
	 * @param tabCaption
	 *            An optional caption which will appear on the Repeater tab
	 *            containing the request. If this value is <code>null</code>
	 *            then a default tab index will be displayed.
	 * @throws java.lang.Exception
	 */
	public void sendToRepeater(String host, int port, boolean useHttps,
			byte[] request, String tabCaption) throws Exception;
	 ...
	 * @throws java.lang.Exception
	 */
	public void sendToIntruder(String host, int port, boolean useHttps,
			byte[] request) throws Exception;
	 ...
	 * @throws java.lang.Exception
	 */
	public void sendToSpider(java.net.URL url) throws Exception;
	 ...
	 * @throws java.lang.Exception
	 */
	public IScanQueueItem doActiveScan(String host, int port, boolean useHttps,
			byte[] request) throws Exception;
	 ...
	 *            A list of index pairs representing the positions of the
	 *            insertion points that should be scanned. Each item in the list
	 *            must be an int[2] array containing the start and end offsets
	 *            for the insertion point.
	 * @return The resulting scan queue item.
	 * @throws java.lang.Exception
	 */
	public IScanQueueItem doActiveScan(String host, int port, boolean useHttps,
			byte[] request, List<int[]> insertionPointOffsets) throws Exception;
	 ...
	 * @throws java.lang.Exception
	 */
	public void doPassiveScan(String host, int port, boolean useHttps,
			byte[] request, byte[] response) throws Exception;
	 ...
	 * 
	 * @param urlPrefix
	 *            This parameter can be used to specify a URL prefix, in order
	 *            to extract a specific subset of the site map. The method
	 *            performs a simple case-sensitive text match, returning all
	 *            site map items whose URL begins with the specified prefix. If
	 *            this parameter is null, the entire site map is returned.
	 ...
	 * @param urlPrefix
	 *            This parameter can be used to specify a URL prefix, in order
	 *            to extract a specific subset of scan issues. The method
	 *            performs a simple case-sensitive text match, returning all
	 *            scan issues whose URL begins with the specified prefix. If
	 *            this parameter is null, all issues are returned.
	 ...
	 *            The handler to be invoked when the user clicks on the menu
	 *            item.
	 */
	public void registerMenuItem(String menuItemCaption,
	 ...
	 * the relevant items in the Map, and then call <code>loadConfig</code> with
	 * the same Map.
	 * 
	 * @param config
	 ...
	 *         suite professional), the major version (e.g. 1.4), the minor
	 *         version (e.g. 03)
	 */
	 ...
	 * 
	 * @param promptUser
	 *            Indicates whether to prompt the user to confirm the shutdown.
	 */
